...
namespace vsnn {
    ...
    public:
        ...
        static void MatMul(const Matrix& X, const Matrix& W, Matrix& Y) {
            ...
            // Zero Y
            for (i32 n = 0; n < Y.Rows(); ++n) for (i32 j = 0; j < Y.Cols(); ++j) Y(n, j) = 0.0f;
            if (W.IsSparse()) {
                // Sparse W: iterate non-zeros (k,j) and accumulate X(:,k) * W(k,j) into Y(:,j)
                for (const auto& kv : W.Sparse()) {
                    const size_t idx = kv.first; const float w = kv.second;
                    const i32 k = static_cast<i32>(idx / static_cast<size_t>(W.Cols()));
                    const i32 j = static_cast<i32>(idx % static_cast<size_t>(W.Cols()));
